...
- `global` → `cli/global-skills.generated.ts` (`GLOBAL_SKILLS`) — machine-level skills (e.g. `harness-config`, `jspace-adopt`), installed only to the per-machine `~/.agents/skills/<name>/` by `skills install` and refreshed by `workspace upgrade` (union view in `cli/commands/skills.ts` `embeddedSkillAssets`).
