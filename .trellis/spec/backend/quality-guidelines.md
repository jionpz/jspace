...
- **External-process ports**: any production dep backed by an external CLI (gbrain) lives in `adapters/` and routes through `adapters/process/spawn.ts` (timeout + output cap + stdin) — a bare `spawnSync` in application is a hang risk for harness hooks. The port is async so a stalled process resolves `{ok:false}` after the timeout instead of blocking (issue #8 #8).
