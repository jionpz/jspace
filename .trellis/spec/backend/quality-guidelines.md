...
- **Harness machine-config paths are SSOT data**: where each harness keeps its gbrain MCP config (path/format/server_key) lives in capabilities.yaml `mcp_config`, consumed by `harness wire`/`gbrain wire` and doctor — never hardcoded in wiring code (issue #8 #16).
