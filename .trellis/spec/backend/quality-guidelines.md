...
- **Incremental ownership model** (parent R4/AC5): bundle files carry `AssetOwnership` (currently all `managed`); `diffBundle` produces `create / no-op / update / conflict / skip / stale`; `materialized.json` is the last-applied base; `workspace upgrade` writes a plan + journal + rollback snapshot under `.jspace/state/upgrades/<id>/`. Locally-modified skills are never force-overwritten (reported as conflict).
