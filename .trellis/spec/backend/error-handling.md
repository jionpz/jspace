...
- **Fail-closed verification**: when a guard cannot verify success (inbox batch log missing / no filehub), report the failure (`batch-stale` incident) **and** record `RunRecord.status` as `"failed"` with `exitCode: 1` — never default to success. Recording `status: "ok"` while opening a batch-stale incident lets `todaySuccess` skip the rest of the day and prints `jspace: ok:` (a fake-success exit; issue #8 #6).
