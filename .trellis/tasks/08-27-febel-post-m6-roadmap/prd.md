...
## 子任务状态表（2026-08-27 综合核对）

| 子任务 | status | 规划物 | 交叉验收结论 |
|---|---|---|---|
| F `08-27-febel-f-pr26-land` | planning | prd.md **仍 TBD 模板（待 F 代理）** | 工程内容由 PR #26 承载；PRD 补全后须含技能列表单源验收 + 合入后 gen-assets/三 check 全绿 |
| E `08-27-febel-e-writeback-flywheel` | planning | prd.md + design.md 完整 | 通过：doctor 离线 info 门禁，零 gbrain 写侧路径、不改 nudge 频率；与 L 分工双向一致 |
| B `08-27-febel-b-platform-ledger` | planning | prd.md 完整（E/H/A 关闭分类 + Disposition 表） | 通过：满足父约束「替代关闭须显式效力边界」；CRUD ≠ fire 红线明确 |
| Eco `08-27-febel-eco-second-machine` | planning | prd.md 完整（P0–P7 协议 + R5 回写槽） | 通过：满足「不伪双机」——物理第二机定义收紧，排除同机/容器/云环境关闭路径 |
| L `08-27-febel-l-usage-flywheel` | planning | prd.md 完整（R1–R3 协议 + M7 草案 + 证据台账模板） | 通过：M7 草案可直接供父任务确认写入；与 E 分界一致 |

## Cross-child Acceptance（2026-08-27 修订）
...
- 父任务 status 保持 planning / 仅在最终集成审视时 start；日常 `task.py start` 对准下一个子任务（当前推荐：**F**）
- 开放 issue/PR：[#26](https://github.com/jionpz/jspace/pull/26)（F 子任务；2026-08-27 核对为 OPEN / MERGEABLE）
