...
test("loadManifestSkillSets matches committed skills-manifest.json", () => {
  ...
  expect(sets.global).toEqual(["harness-config", "jspace-adopt"]);
  expect(sets.total).toBe(9);
});
...
test("parseSkillListingLine extracts slash-separated workbench names (README style)", () => {
  const line =
    "skills/` —— 官方技能源码（7 个 workbench 技能：jspace-use / asset-ingest / memory-recall；另有 global 段的 `harness-config` 与 `jspace-adopt` 技能，manifest 合计 9 个）";
  const listing = parseSkillListingLine(line);
  expect([...listing.names].sort()).toEqual(["asset-ingest", "harness-config", "jspace-adopt", "jspace-use", "memory-recall"]);
  expect(listing.totalClaimed).toBe(9);
});

test("parseSkillListingLine extracts backtick workbench names (AGENTS style)", () => {
  const line =
    "`skills/`：… 当前 7 个 workbench 技能——`jspace-use`（指南）、`asset-ingest`（入库）；另有 2 个 `scope: global` 的机器级技能 `harness-config`、`jspace-adopt`（…）。manifest 合计 9 个。";
  const listing = parseSkillListingLine(line);
  expect([...listing.names].sort()).toEqual(["asset-ingest", "harness-config", "jspace-adopt", "jspace-use"]);
  expect(listing.totalClaimed).toBe(9);
});

test("diffSkillNameSets reports missing and extra names", () => {
  const expected = ["jspace-use", "asset-ingest", "harness-config", "jspace-adopt"];
  const found = new Set(["jspace-use", "phantom-skill"]);
  const { missing, extra } = diffSkillNameSets(expected, found);
  expect(missing).toEqual(["asset-ingest", "harness-config", "jspace-adopt"]);
...
test("checkDocSkillListing fails when prose claims the wrong manifest total", () => {
  ...
  const readme = readFileSync(join(ROOT, "README.md"), "utf-8");
  const drifted = readme.replace("manifest 合计 9 个", "manifest 合计 10 个");
  const failures = checkDocSkillListing(drifted, "README.md", sets);
  expect(failures.some((f) => f.includes("manifest 合计 claims 10"))).toBe(true);
});
