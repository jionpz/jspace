...
const globalSkillDirs = skillsManifest.global.map((s) => `skills/${s.name}`);
for (const d of globalSkillDirs) {
  if (!statSync(join(repoRoot, d), { throwIfNoEntry: false })?.isDirectory()) {
    throw new Error(`skills-manifest declares missing skill dir: ${d}`);
  }
}
...
// Machine-global skills (issue #37) walk into a SEPARATE map, never into
// ASSETS — the workbench materialization machinery (materializedRels /
// materializeTree / diffBundle) reads only ASSETS, so global content is
// structurally excluded from .jspace/skills/ and harness projections.
const globalAssets = new Map<string, string>();
for (const d of globalSkillDirs) walk(join(repoRoot, d), join(repoRoot, d), globalAssets);
...
// Global-skill content map (issue #37): installed to ~/.agents/skills/<name>/
// by `skills install` / `workspace upgrade`. Keys mirror repo paths so the
// reachability reference-resolution helpers work over both maps unchanged.
const globalBody = [...globalAssets.entries()]
  .map(([k, v]) => `  ${JSON.stringify(k)}: ${JSON.stringify(v)},`)
  .join("\n");
const globalSkillsOutput = `// generated by scripts/gen-assets.ts — do not edit by hand.
// Regenerate with: bun run scripts/gen-assets.ts
// Machine-global skills (manifest.global): installed to ~/.agents/skills/<name>/
// by \`jspace skills install\`, refreshed by \`workspace upgrade\`. Deliberately
// separate from ASSETS so workbench materialization can never see it (issue #37).
export const GLOBAL_SKILLS: Record<string, string> = {
${globalBody}
};
`;
writeFileSync(join(repoRoot, "cli", "global-skills.generated.ts"), globalSkillsOutput, "utf-8");
console.log(`wrote cli/global-skills.generated.ts (${globalAssets.size} files)`);
