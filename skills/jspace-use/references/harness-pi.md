...
- **文件式（推荐）**：`~/.pi/agent/SYSTEM.md`（替换默认 system prompt）与 `~/.pi/agent/APPEND_SYSTEM.md`（追加），或项目 `.pi/SYSTEM.md`；context files（AGENTS.md/CLAUDE.md）本身在启动时注入。
- **扩展事件式**：`session_start`（reason = startup/reload/new/resume/fork）与 `before_agent_start`（可注入消息/改 system prompt）——需 pi-mcp-adapter 提供，**未实测**。
- **手动兜底**：`jspace context session-start --plain` 输出可手动贴进会话（无需 hook）。

## 能力边界（诚实声明）

- **无自动 session-start context 注入**：Pi 无 hooks.json，会话开始不会自动跑 `jspace context session-start`。要么靠 SYSTEM.md 静态提示，要么手动跑 `--plain` 贴入。
...
```bash
jspace doctor --dir .          # checkHarness 的 Pi 分支: wired(CLI 可用) / missing(CLI 不可用)
