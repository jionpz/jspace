...
## 4. 决策原则
> 决策顺序是强约束;安全、隐私、数据完整性与不可逆性始终是更高优先级的 invariant。

- **先做第一性原理推理**:先说清目标、约束、已知事实和不可破坏的 invariant,再讨论工具或实现。
- **其次用奥卡姆剃刀**:在所有满足 invariant 的机制中,选择最小、最少部件、最少状态、最少配置的方案。
- **最后验证**:用可复现的测试或检查证明方案满足 invariant,并确认最小方案没有掩盖错误的问题定义。
- **强制顺序**:定义 invariant → 最小化机制 → 验证;不得跳步或以实现便利性倒推问题定义。
- 安全、隐私、数据完整性、不可逆性是更高优先级的约束;不得以"简化"为名移除必要的防护、审计或恢复能力。
- **反模式**:工具先行;配置 / 状态 / 服务不断堆叠;为假设的未来需求预留扩展点;把复杂度当作健壮性;用"简单"删掉必要安全约束;用额外复杂度掩盖错误的问题定义;先定方案再倒推问题定义。
