...
**幂等(`.processing`)**:处理某份前在 `_inbox/` 写 `<file>.processing`(点文件,doctor/计数忽略);**成功或失败都删除标记**(瞬态锁,防并发处理同一份)。
- 中断续跑:仍有 `.processing` 的 = 正在处理或上次被中断,重跑时重新尝试;已完成(成功归位)的文件已移出 `_inbox/`,天然不重复。
- **失败重试**:失败的文件留在 `_inbox/`,原因记入汇总与执行日志 → 下次批量(含 cron 无头)自然重试,不永久跳过。
