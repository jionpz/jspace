...
- **M7（使用里程）⏳ 开放** — 建造→使用的拐点验收槽（起草于 FEBEL-L，2026-08-27 入 GOAL）：机制已齐（M6 + B4 写回接线 + 来源 tag + H1 硬化）之后，用**真实使用**证明三个飞轮在转，而不是再用工程厚度代替转速。**协议已发行**：`skills/jspace-use/references/usage-mileage.md`（随 init/upgrade 物化，含三腿关闭条件、取证口径、禁伪造红线）；可复制台账 **`usage-mileage-ledger-template.md`** → 本机 `<wb>/.jspace/usage-mileage-ledger.md`（gitignore，填真实证据后不回填仓库）。
  ...
  - **记忆腿（写回习惯）**：连续两周 `gbrain list --tag source:session` 落窗口计数 > 0（口径同 `workbench-retro` 检查 1）。**禁止伪造**；当前为 0 且属预期（来源 tag 是新约定，此前的页都没有）时保持开放。到期把真实 `session_writes` / 窗口日期 / 对应 retro 页 slug 回写本条。**进度（2026-09-17，未完结）**：W1（09-14~09-20，截至 09-17）业务写入 **42**（`project/tugo` 33 + 非 tugo 9）/ 工具 5 / `source:cron` 窗口内 0 → 首周信号 **> 0 成立**；W2（09-21~09-27）未到，本腿保持开放。**同时暴露一条口径问题（待裁决）**：红线 1 字面「不给历史页补 tag」与 09-17 对 tugo 历史页补全出处标签的动作冲突；取证表明那 33 条 tugo 页中 31 条的**页内 frontmatter `source:` 在写入当时就写明 `session <日期>`**（`updated_at` 与之一致），故属「把已声明的出处变可查询」而非制造出处；且首周达标**不依赖该子集**（非 tugo 9 条独立支撑）。建议把红线改为「不得补 tag 以制造该页**未曾声明**的出处」——**未擅改，待用户裁决**。明细见台账 §3「W1 标签完整性复核」。
