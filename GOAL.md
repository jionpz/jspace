...
- **M7（使用里程）⏳ 开放** — 建造→使用的拐点验收槽（起草于 FEBEL-L，2026-08-27 入 GOAL）：机制已齐（M6 + B4 写回接线 + 来源 tag + H1 硬化）之后，用**真实使用**证明三个飞轮在转，而不是再用工程厚度代替转速。**协议已发行**：`skills/jspace-use/references/usage-mileage.md`（随 init/upgrade 物化，含三腿关闭条件、取证口径、禁伪造红线）；可复制台账 **`usage-mileage-ledger-template.md`** → 本机 `<wb>/.jspace/usage-mileage-ledger.md`（gitignore，填真实证据后不回填仓库）。
  - **自省腿**：真实工作台完成无头 `workbench-retro` 首跑（rehearsal `jspace cron run workbench-retro` → `records/retro/<YYYY-MM-DD>` 含写回率一节与逐条证据行、`tags` 含 `source:cron`、doctor 未因 retro 变坏）。可选加强：周日 23:00 自然触发一次并记账。**这里的 `cron run` 只支撑「skill 无头跑通」这一 claim，不用于关闭开放问题 #5 的「真实触发」**（后者要真机被调度器到点拉起）——两者回写时分开表述。
  ...
  - **R1 无头首跑已采（2026-09-12，本机 `~/jspace-work`）**：`jspace cron run workbench-retro --force --timeout 900` → exit 0，log `.jspace/logs/cron/workbench-retro/2026-09-12T194015-b1d1b41f.md`（1058 行），retro 页 `records/retro/2026-09-12` 含「写回率」一节；doctor 跑后 0 error / 0 warning / 1 info、未改工作台文件。**自省腿仍未关闭**：首跑来源 tag 由人工纠正后才合规；修复后复跑已自动合规（见下），但「自然触发」（周日 23:00 由调度器拉起）尚未发生。台账 §2a / §2a-1。
