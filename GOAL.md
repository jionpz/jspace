...
5. **Linux/Windows 真机调度复核**——**未验证（截止 2026-08-26）**：CI 已覆盖三平台构建/冒烟（init+doctor）/一键安装闭环（`.github/workflows/build.yml`），但 scheduler 真机行为未复核：Linux crontab 安装/无补跑语义、Windows schtasks 登录态/登出边界、Linux 无 crond fail-fast 与沙盒 namespace 降级。已知缺口：Windows x64 发布二进制非 baseline（需 AVX2，GH runner 下载 baseline runtime 被阻断；AVX-less 构建仅本地 `bun run build:all` 产出）；CI cron 冒烟仍 `if: false` 占位。复核程序与通过标准见 `docs/PLATFORMS.md`（手动验证矩阵 + doctor 断言表）。**执行后回写**：本条目状态 + `docs/PLATFORMS.md`「真机验证台账」对应行（日期 + 证据），并视结果解锁 CI cron 冒烟。
